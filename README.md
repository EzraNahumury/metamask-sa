...
*An autonomous reasoning agent that pays your subscriptions over x402, refuses anomalies in real time, and produces a Friday Brief in three Venice modalities — all on Base mainnet via MetaMask Smart Accounts + 1Shot's permissionless relayer.*
...
DeleGate.AI is an autonomous money-management agent. You give it a weekly USDC budget through an **ERC-7715** advanced permission on a **MetaMask Smart Account** (upgraded to a smart account via **ERC-7702**). Every tick, the agent walks a list of subscription services, fetches an **HTTP 402** quote from each, asks **Venice** to reason about whether the price is sane, and either pays the merchant by signing an **EIP-3009** USDC transfer or refuses with an explanation. Approved payments are settled onchain through a real **ERC-7710 redelegation** broadcast by the **1Shot permissionless relayer** — gas paid in USDC, no ETH anywhere. Every Friday the agent compiles a **Friday Brief** using three Venice endpoints — text reasoning, a generated chart image, and a 60-second voice narration. You can revoke at any time, on-chain, in one tap.
...
1. **Live indicator** turns green when the dashboard connects to the agent over SSE.
2. **Stat cards** — Paid, Refused, Services, Decisions.
3. **Demo controls** — *Run tick now* fires one reasoning + payment pass; per-service *Arm <slug>* buttons inflate the next quote 32× so you can demo the anomaly-catch moment live.
4. **Friday Brief** — one button generates the multi-Venice digest (text + image + audio).
5. **Live decision feed** — every PAY / REFUSE / ESCALATE appears in real time with the agent's reasoning, a receipt id, and a clickable basescan link when settled onchain.

### 2.3 The 90-second demo arc

| Time | Action |
|---|---|
| 0:00 | Open dashboard — agent already connected via SSE. |
| 0:10 | Hit **Run tick now** — five services queried, Venice reasons, five PAY decisions appear with receipts. |
| 0:30 | Hit **Arm netflix-mock** — frontend POSTs to merchants admin endpoint. |
| 0:35 | Hit **Run tick now** again — Netflix-mock quote returns $511.68. |
| 0:40 | Agent REFUSES Netflix-mock with reason *"price exceeds per-call cap"*; other services PAY normally. |
| 0:55 | Click a basescan link on any PAY row — see the real onchain ERC-7710 redemption + USDC transfers. |
| 1:10 | Hit **Generate Friday Brief** — wait ~10 s, watch text + chart image + audio render. |
| 1:30 | Press play on the audio — voice narrates the week. |

---

## 3. Onchain evidence (Base mainnet — verifiable now)

These are real transactions broadcast by the public 1Shot relayer on behalf of the burner smart account `0x5Aea061d814A72de9EE9171bE86F45f48e1E2f5d`:
...
| **MetaMask ERC-7715 Advanced Permissions** | `apps/agent` reads the budget as if it came from a 7715 grant (`PER_CALL_CAP`, `WEEKLY_BUDGET_MICRO_USDC`, `allowedCallees`). Day-4 UI work wires the actual extension prompt; the agent honors the caveats today. |
...
| - Demo controls (Run tick / Arm anomaly)                     |
...
- `oneshot/relayer.ts` — JSON-RPC client (`getCapabilities`, `getFeeData`, `send7710Transaction`).
...
cp .env.example .env
# Fill in:
#   SPIKE_PRIVATE_KEY      — burner EOA (never reuse personal keys)
#   ONCHAIN_SETTLEMENT_MODE=dust   (optional; otherwise mock-only)
#
...
The Day-0/1 verification scripts that proved every primitive in isolation are still in `spike/` and runnable: `pnpm spike:1shot`, `pnpm spike:venicex402`, `pnpm spike:first7710`, etc. See [`SPIKE.md`](./SPIKE.md) for the canonical schemas they discovered.
...
- The agent uses the *same* burner key as the "user" — in production, the user signs an ERC-7715 grant in the MetaMask extension and the agent runs as a separate session account. Today, both roles share one EOA for simplicity. The 7715 caveats (per-call cap, weekly budget, allowed callees) are enforced inside `apps/agent` rather than by the framework.
- The anomaly trigger is a `POST /admin/arm-anomaly/<slug>` button — the rule engine that detects it is real (median + per-call cap comparison; the model also evaluates).
...
- [x] Working integration shown in demo (dashboard live).
...
- [x] Canonical x402+7710 facilitator pattern lives in `packages/core/src/oneshot/settle.ts` — reusable for any team.
...
| Status | Item |
|---|---|
| ✅ | Day-0 primitive verification (spike 01–08) |
...
2. **[`SPIKE.md`](./SPIKE.md)** — every architectural finding from Day-0/1 (canonical schemas, address pinning, version pins).
3. **[`PROPOSAL.md`](./PROPOSAL.md)** — the original strategic deep dive: idea exploration across 15 candidates, why DeleGate.AI won the deliverability pass, judge-level critique with mitigations.
4. **[`requirement.md`](./requirement.md)** — the hackathon brief, mirrored verbatim.
